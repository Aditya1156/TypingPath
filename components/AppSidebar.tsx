...
interface AppSidebarProps {
  ...
  onNavigate: (view: 'chapters' | 'chapter' | 'lessons' | 'guide' | 'dashboard') => void;
  activeView: string;
}
...
const AppSidebar = ({ isOpen, onClose, onGoToHome, onShowModal, onNavigate, activeView }: AppSidebarProps) => {
  ...
  const NavLink = ({ view, children, icon }: { view: 'chapters' | 'lessons' | 'dashboard', children: React.ReactNode, icon: React.ReactNode }) => {
    const isActive = activeView === view || 
      (activeView === 'test' && (view === 'lessons' || view === 'chapters')) || 
      (activeView === 'guide' && (view === 'lessons' || view === 'chapters')) ||
      (activeView === 'chapter' && view === 'chapters');
    return (
      <button 
  ...
  return (
    <aside className={`fixed top-0 right-0 h-full w-64 bg-secondary border-l border-border-primary flex flex-col p-4 z-[60] transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
      ...
      <nav className="flex flex-col gap-2">
        ...
        <NavLink view="chapters" icon={<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg>}>
          Chapters
        </NavLink>
